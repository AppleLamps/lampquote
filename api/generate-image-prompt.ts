...
import { poeChatJson, getAssistantText, parseDataUrl } from "../lib/poe-server";
...
Output only the final image prompt, nothing else.`,
    ...
    return res.status(200).json({ prompt: optimizedPrompt });
  } catch (e: unknown) {
    const err = e as { status?: number; message?: string };
    const status = err.status && err.status >= 400 && err.status < 600 ? err.status : 500;
    const message = e instanceof Error ? e.message : "Failed to generate image prompt";
    if (status === 429) {
